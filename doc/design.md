...
### 附录：前端设计原则（2026-01-03）

1. **信息只出现一次**：任何字段仅会出现在一个列中，避免重复输入或显示；Hero 也只保留一句价值主张。
2. **强制分栏语义**：左=系统、 中=输入、右=结果，不允许跨列控件，滚动时各列互不干扰。
3. **高频子组件化**：头像、脚本等复杂区域抽象为子组件（例如 `AvatarConfigurator`、`ScriptEditor`），父组件只负责 orchestrate 与折叠状态。
4. **默认折叠低频内容**：高级参数、日志抽屉、素材详情等一律默认折叠，界面保持清爽；用户手动展开后通过 `localStorage` 记忆。
5. **Trace 可追溯**：任务列表、日志区均突出 trace id，配合后端 `[ISO][LEVEL][trace=xxx]` 规范，便于支撑与计费核算。
