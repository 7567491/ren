...
def generate_images_parallel(shots, shot_count, character_image_path=None):
    ...
    def generate_with_flag(shot_desc, shot_id, shot_cnt, char_img_path):
        """包装函数：为第一个任务设置详细日志标志"""
        ...
        return generate_image(shot_desc, shot_id, shot_cnt, char_img_path, verbose=is_first)

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 提交所有任务
        future_to_shot = {
            executor.submit(generate_with_flag, shot['description'], shot['id'], shot_count, character_image_path): shot
            for shot in shots
        }
    ...
    def generate_video_with_retry(shot, image_url, max_retries=3):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                video_file = generate_video(image_url, shot['description'], shot['id'], shot_count, resolution, camera_movement)
...
def generate_image(prompt, shot_id, shot_count, character_image_path=None, verbose=True):
    """使用配置的模型生成图像（支持参考图）
    ...
    if character_image_path:
        prompt_text = f"{prompt.strip()} 保持人物面部特征与人物参考图完全相同"
...
@smart_retry(max_attempts=3, backoff_factor=2)
def generate_video(image_url, prompt, shot_id, shot_count, resolution="720p", camera_movement=None):
...
def main():
    ...
    parser.add_argument('--config-file', type=str, metavar='CONFIG_PATH',
                       help='指定配置文件路径 (默认: user.yaml，API模式: temp/user-{job_id}.yaml)')
    ...
    if config_result:
        ...
        # 检查是否在非交互式环境中运行（如API后台进程）
        if not sys.stdin.isatty():
            # 非交互式环境，不能使用input()
            print(f"\n{Colors.RED}❌ 错误：配置文件验证失败，且当前运行在非交互式环境（API后台模式）{Colors.RESET}")
