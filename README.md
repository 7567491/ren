...
#### 配置文件层级
- **`.env`**：仅存放 `DeepSeek_API_KEY` 等 LLM/TTS 密钥，Wavespeed API Key 由前端输入后写入 `user.yaml`，不提交。
...
**A:** 先运行 `python3 py/install_fonts.py`，脚本会下载 Google Noto Sans SC 到 `resource/fonts/` 并写入配置。若依旧看到“Arial 不可用”或字幕乱码：
1. 确保运行机器可以访问 `github.com` 以便自动下载字体（或在有网环境运行脚本后拷贝字体目录）
2. 也可以手动将 `.ttf/.otf/.ttc` 中文字体文件放入 `resource/fonts/` 目录，程序会自动识别
3. 若本地已有系统字体，更新 `config.yaml > subtitles.default_style.font_candidates` 以包含真实路径

完成一次字体下载后会缓存本地，无需重复配置。
