...
def load_config_from_yaml(config_path='user.yaml'):
    ...
    try:
        ...
        if character.get('enabled', False):
            ...
            has_description = character.get('description', '')
            has_description = has_description.strip()
...
def validate_config():
    ...
    if not WAVESPEED_API_KEY:
        errors.append("缺少 Wavespeed API 密钥（请通过前端输入或在 user.yaml 的 api.wavespeed_key 中配置）")
