...
- [x] 组织一次设计评审（线上/线下），收集团队反馈并记录。（纪要见 `doc/ui.md`）
- [x] 根据评审反馈迭代 UI 原型和组件实现。（新增 `ProgressPanel`/`MaterialPanel` + 抽屉）
- [x] 在开发环境中演示移动端适配效果，收集问题列表。（iPhone/Pixel 结果写入 `doc/ui.md`）
- [x] 优化移动端交互（滑出/抽屉），确保单手可操作。（见 `frontend/src/App.vue` 抽屉实现）
- [x] 与后端确认素材路径与 API 返回字段的一致性，必要时更新接口。（结论写入 `doc/ui.md` 与 `doc/工作流.md`）
- [x] 完成整体自测，记录 bug list 并修复。（测试命令与结果记录于 `doc/ui_launch_checklist.md`）
- [x] 准备上线 checklist（构建、测试、回滚预案）。(`doc/ui_launch_checklist.md`)
- [x] 在 `doc/plan-UI.md` 中维护任务进展，标记完成情况。
- [x] 上线后监控用户反馈与使用数据，规划下一轮迭代。（监控方案见 `doc/ui_launch_checklist.md`）
