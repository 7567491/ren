    ...
    <div class="workspace">
      <section v-show="currentView === 'setup'" class="workspace__column workspace__column--system">
        <PreparationPanel
          ...
          @refresh-history="refreshHistoryVideos"
        />
      </section>
...
import type { HistoryVideoItem, HistoryVideoResponse } from '@/types/history';
...
const voiceId = ref('Chinese (Mandarin)_Reliable_Executive');
...
const historyVideos = ref<HistoryVideoItem[]>([]);
...
async function handleSubmit() {
  ...
  // 验证 API Key
  const apiKey = dashboardStore.apiKey.trim();
  if (!apiKey || apiKey.length < 10) {
    return showFormAlert('请先在"环境配置"中输入 Wavespeed API Key');
  }
  ...
  const payload: Record<string, any> = {
    ...
    wavespeed_api_key: apiKey  // 添加用户的 API Key
  };
...
async function refreshHistoryVideos() {
  historyLoading.value = true;
  historyError.value = '';
  try {
    const response = await fetch(`${config.API_BASE}/api/history/videos?limit=12`, {
      headers: buildRequestHeaders()
    });
    if (!response.ok) {
      const error = await safeJson(response);
      throw new Error(error.detail || '加载历史视频失败');
    }
    const payload: HistoryVideoResponse = await response.json();
    historyVideos.value = payload.items ?? [];
  } catch (err) {
    historyVideos.value = [];
    historyError.value = (err as Error).message || '加载历史视频失败';
  } finally {
